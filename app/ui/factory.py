...
def create_button(text, color, icon=None, expand=True, height=50, on_click=None):
    ...
    if icon:
        content_controls.append(ft.Icon(name=icon, color=text_color))
    if text:
        content_controls.append(ft.Text(value=text, color=text_color))

    if len(content_controls) > 1:
        content = ft.Row(controls=content_controls, alignment=ft.MainAxisAlignment.CENTER, spacing=5)
    elif content_controls:
        content = content_controls[0]
    else:
        content = None
