...
from app.handler.steps.translate_queue import process_folder_queue  # import directly from handler to avoid circular import 
...
async def step_1_split(handler, path_service, base_name, filename):
    ...
    if not os.path.exists(split_folder):
        log.write(f"❌ 1단계 실패: 분리 폴더가 생성되지 않았습니다. ({split_folder})")
