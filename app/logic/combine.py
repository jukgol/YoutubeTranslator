...
async def combine_parts_logic(folder_name, translate_dir, combine_dir, log_callback):
    try:
        ...
        if not os.path.exists(source_folder):
            log_callback(f"❌ 폴더를 찾을 수 없습니다: {source_folder}")
            return False
        
        files = sorted([f for f in os.listdir(source_folder) if f.lower().endswith('.txt')])
        if not files:
            log_callback(f"⚠️ {folder_name} 폴더에 합칠 파일이 없습니다.")
            return False

        log_callback(f"🚀 {folder_name} 파트 합치기 시작 (총 {len(files)}개 파트)")
        ...
        log_callback(f"✅ 합치기 완료: {output_path}")
        return True

    except Exception as e:
        log_callback(f"❌ 작업 중 에러 발생: {str(e)}")
        return False