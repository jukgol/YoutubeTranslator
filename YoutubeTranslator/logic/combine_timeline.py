...
def check_missing_indices(file_path, label, log_callback):
    ...
    try:
        ...
        if missing:
            log_callback(f"❌ {label} 내에 빠진 번호가 있습니다: {missing}")
            return False
        return True
    except:
        return False
...
def combine_timeline_logic(combined_text_path, origin_srt_path, result_dir, log_callback):
    try:
        ...
        log_callback(combined_text_path +"\n");
        log_callback(origin_srt_path +"\n");

        check_a = check_missing_indices(origin_srt_path, "원문(A)", log_callback)
        check_b = check_missing_indices(combined_text_path, "번역문(B)", log_callback)
        
        if not (check_a and check_b):
            log_callback("⚠️ 인덱스 번호가 일치하지 않아 작업을 중단합니다. 파일을 수정해 주세요.")
            return False
        # ----------------------------------------------
        ...
        validate_translated_indices(raw_sections, log_callback)
        ...
        # ------------------------------
        # 3. 상세 비교 로그 출력
        log_callback(f"📊 [비교 시작] 타임라인(A) 개수: {len(timestamps)}개 / 번역문(B) 개수: {len(translated_lines)}개")

        if len(timestamps) != len(translated_lines):
            log_callback("❌ [불일치 상세 정보]")            
            return False
        ...
        log_callback(f"✅ 최종 자막 생성 완료: {output_path}")
        return True

    except Exception as e:
        log_callback(f"❌ 결합 에러: {str(e)}")
        return False


def validate_translated_indices(raw_sections, log_callback):
    """
    ...
    log_callback(f"🔎 번역본 인덱스 순서 검사 시작 (총 {len(raw_sections)}개 문단)")
    ...
    for i, section in enumerate(raw_sections):
        ...
        if first_line.isdigit():
            ...
            if current_idx != expected_idx:
                log_callback(f"❌ 순서 오류: {i}번째 문단에서 번호 {expected_idx}를 예상했으나 {current_idx}가 발견되었습니다.")
                success = False
                # 다음 검사를 위해 인덱스를 실제 발견된 번호 기준으로 강제 동기화 (옵션)
                expected_idx = current_idx
        else:
            log_callback(f"⚠️ 경고: {i}번째 문단이 숫자로 시작하지 않습니다. 내용 일부: '{first_line[:15]}...'")
            success = False
        
        expected_idx += 1

    if success:
        log_callback("✅ 모든 번역본 인덱스가 순서대로 잘 배치되어 있습니다.")
    else:
        log_callback("⚠️ 인덱스 불일치가 발견되었습니다. 파일을 확인해 주세요.")
